...
export function CitizenCard({ citizen, className }: { citizen: Citizen; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 border-t-ink border-ink px-3 py-2">
        <div className="min-w-0">
          <p className="font-display text-lg leading-none">#{citizen.id}</p>
          <p className="mt-1 truncate font-body text-xs text-brown">
            {citizen.gender} · {citizen.mood}
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          {rare && <RarityTag label="Rare" />}
          <DownloadMenu citizen={citizen} preventPropagation size="sm" />
        </div>
      </div>
    </Link>
  );
